...
object PCGTest {
  ...
  case class Particle(var p: Vec2, var v: Vec2) {
  ...
  def particlesSVG(root: html.Div): Unit = {
    ...
    import scalatags.JsDom.svgTags.{svg, path, g}
    import scalatags.JsDom.svgAttrs.{d, fill, stroke, width, height, transform}
    val particles = mutable.Buffer[Particle]()
    ...
    def field(p: Vec2) = Vec2(noiseX.simplex2(p.x / 50, p.y / 50), noiseX.simplex2(p.x / 50 + 1, p.y / 50 + 1)) * 0.5
  ...
  def particles(canvas: html.Canvas): Unit = {
    ...
    //for (i <- 1 to 100) particles += Particle(Rand.withinAABB(screen), Rand.withinCircle())
    particles ++= Circle2(Vec2(canvas.width / 2.0, canvas.height / 2.0), 100).toPolygon(100).points.map(v => Particle(v, Vec2(0, 0)))
    ...
    def update(dt: Double): Unit = {
      for (p <- particles) {
        p.p += p.v * dt
        p.v += Vec2(noiseX.simplex2(p.p.x / 100, p.p.y / 100), noiseY.simplex2(p.p.x / 100, p.p.y / 100))
      }
    }
    def render(): Unit = {
      for (p <- particles) {
  ...
  */
  ...
  def noise(canvas: html.Canvas): Unit = {
    ...
    for (y <- 0 until ctx.canvas.height; x <- 0 until ctx.canvas.width) {
      ...
      val v: Int = (Math.abs(d) * 256).toInt
